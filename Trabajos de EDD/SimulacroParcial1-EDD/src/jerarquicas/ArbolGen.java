    ...
    private boolean perteneceAux(Object elemento, NodoGen nodo) {
        ...
        if (nodo != null) {
            if (nodo.getElem().equals(elemento)) {
                ...
                if (!pertenece) {//modificado por recomendacion profe, antes pertenece==false
         ...
         if (nodo != null) {    
           ...
           while(unHijo!=null && !exito){ 
               if(unHijo.getElem().equals(hijo)){   
                        padre=nodo.getElem();
                        exito=true;//modifica por recomendacion de la profe, antes unHijo=null        
           }else{
...
//listaPreorden
    ...
    public Lista listarNiveles() {
        ...
        while (!c.esVacia()) {
            ...
            c.sacar();
            contLista=l.longitud()+1;
            l.insertar(nodo.getElem(), contLista);
            if (nodo.getHijoIzquierdo() != null) {
    ...
    private boolean verificarPatronAux(NodoGen nodo, Lista ls, int p) {
        boolean verifica = false;
        System.out.println("verifica: "+verifica);
        if (nodo != null) {
            Object elemento = ls.recuperar(p);
             System.out.println("nodo: "+nodo.getElem());
            
            if (nodo.getElem().equals(elemento)) {
                System.out.println("contador: "+p);
                if(ls.longitud() == p && nodo.getHijoIzquierdo()==null){
                   
                    verifica=true;
                }else{
           
              verifica=  verificarPatronAux(nodo.getHijoIzquierdo(), ls, p+1);
                   
                  if(!verifica){
                verifica = verificarPatronAux(nodo.getHermanoDerecho(), ls, p+1);
                  }
    ...
    // Este metodo retorna una lista con los nodos hojas de un arbol generico 
        Lista lista=new Lista();
    ...
    private int misteriosoAux(NodoGen n) {
        ...
        System.out.println("retorna "+aux);
        return aux;
