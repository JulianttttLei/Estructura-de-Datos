...
//ordenada con los elementos que se encuentran en el intervalo [elemMinimo,elemMaximo].
    ...
    private void obtenerListaPreorden(NodoABB nodo, Lista l) {
        // Este es un metodo recursivo privado 

        // se inserta el elemento del nodo a la lista
        l.insertar(nodo.getElem(), l.longitud() + 1);

        if (nodo.getIzquierdo().getElem().compareTo(nodo.getElem())<0) {
            obtenerListaPreorden(nodo.getIzquierdo(), l);
        }
        if (nodo.getDerecho().getElem().compareTo(nodo.getElem())>0) {
